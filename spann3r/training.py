...
from dust3r.utils.image import imread_cv2, ImgInvNorm
...
def get_args_parser():
    ...
    # Training
    parser.add_argument('--batch_size', default=4, type=int, # ORIGINAL: 2
    ...
    # others
    parser.add_argument('--num_workers', default=4, type=int) # ORIGNAL: 1
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    optimizer.zero_grad()
    vis_idx, save_interval = 0, 20
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        epoch_f = epoch + data_iter_step / len(data_loader)

        if (data_iter_step + 1) % accum_iter == 0 and ((data_iter_step + 1) % (accum_iter * args.print_freq)) == 0 and misc.is_main_process():
            # visualize the data
            visualize_batch(batch, f'{epoch:03d}/CurrentSave{vis_idx}', args)
            vis_idx += 1
            if vis_idx == save_interval:
                vis_idx = 0
...
def visualize_batch(batch, cur_save_name, args, img_idx=0):
    save_path = os.path.join(args.output_dir, 'vis_data', cur_save_name)
    if not os.path.exists(save_path):
        os.makedirs(save_path, exist_ok=True)

    label_info = batch[0]['dataset'][img_idx] + ': ' + batch[0]['label'][img_idx]
    with open(os.path.join(save_path, 'data_info.txt'), 'w') as f:
        f.write(label_info)
    imgs = np.concatenate([ImgInvNorm(view['img'][img_idx])*255 for view in batch], axis=1)
    all_pts3d = np.concatenate([view['pts3d'][img_idx][view['valid_mask'][img_idx]] for view in batch], axis=0)
    all_pts3d_rgb = np.concatenate([ImgInvNorm(view['img'][img_idx])[view['valid_mask'][img_idx]] for view in batch], axis=0)

    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(all_pts3d)
    pcd.colors = o3d.utility.Vector3dVector(all_pts3d_rgb)
    cv2.imwrite(os.path.join(save_path, 'puzzles.png'), imgs[...,::-1])
    o3d.io.write_point_cloud(os.path.join(save_path, 'pointcloud.ply'), pcd)
