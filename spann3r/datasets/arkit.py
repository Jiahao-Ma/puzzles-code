...
class ArkitScene(BaseManyViewDataset):
    def __init__(self, num_seq=100, num_frames=5, 
                 ...
                 kf_every=1, train_ratio=None, load_lmdb=True, json_file=None, *args, ROOT, **kwargs):
        self.ROOT = ROOT
        ...
        if self.load_lmdb:
            if json_file is None:
                json_file = osp.join(self.ROOT, 'ARKitScenes.json')
            else:
                json_file = osp.join(self.ROOT, json_file)
            with open(json_file, 'r') as f:
                self.arkit_json = json.load(f)
            
            lmdb_path = osp.join(self.ROOT, 'ARKitScenes.mdb')   
            ...
            self.data_mdb = lmdb.open(lmdb_path,
                                            ...
                                            readahead=False,
                                            map_size=map_size)
            self.data_prefix = 'ARKitScenes/raw'
        else:
            self.blendedmvs_json = None
            self.blendedmvs_mdb = None
    ...
    def _load_jsonl(path):
        objs = []
        with open(path) as f:
            for ln in tqdm(f, desc=f'load {osp.basename(path)}'):
                ln = ln.strip()
                if ln:
    ...
    def load_all_scenes(self, base_dir, num_seq=200):
        ...
        if self.test_id is None:
            if not self.load_lmdb:
                ...
                self.scene_path = osp.join(self.data_prefix, self.mode)
                self.scene_list = list(self.arkit_json[self.mode].keys())
    ...
    def get_intrinsic(self, intrinsics_dir, frame_id, video_id):
        ...
        if not self.load_lmdb:
            ...
            with self.data_mdb.begin(write=False) as txn:
                intrinsic = txn.get(osp.join(intrinsics_dir, f"{video_id}_{frame_id}.pincam").encode('utf-8'))
                intrinsic = intrinsic.decode('utf-8').split('\n')[0].split()
                fx, fy, hw, hh = [float(x) for x in intrinsic[2:]]
                intrinsic = np.asarray([[fx, 0, hw], [0, fy, hh], [0, 0, 1]])
                return intrinsic
    ...
    def _get_views(self, idx, resolution, rng, attempts=0): 
        ...
        if not self.load_lmdb:
            ...
            img_idxs_ = sorted(self.arkit_json[self.mode][scene_id])
            img_idxs_ = [osp.basename(x).split(".png")[0].split("_")[1] for x in img_idxs_]
        ...
        if not self.load_lmdb:
            ...
            with self.data_mdb.begin(write=False) as txn:
                traj = txn.get(osp.join(self.scene_path, scene_id, 'lowres_wide.traj').encode())
                if traj is None:
                    with open('/scratch3/ma040/dataset/LMDB/incomplete.txt', 'a') as f:
                        f.write(f"arkitscenes {scene_id} {osp.join(self.scene_path, scene_id, 'lowres_wide.traj')}\n")
                    new_idx = rng.integers(0, self.__len__()-1)
                    return self._get_views(new_idx, resolution, rng)
                traj = traj.decode('utf-8').split('\n')
                for line in traj:
                    if line.strip():
                        poses_from_traj[f"{round(float(line.split(' ')[0]), 3):.3f}"] = np.array(
                            self.traj_string_to_matrix(line)[1].tolist()
        ...
        while len(imgs_idxs) > 0:
            ...
            if not self.load_lmdb:
                ...
                with self.data_mdb.begin(write=False) as txn:
                    try:
                        ...
                        print(f"[ARKitScenes] Error loading {impath} or {depthpath}: {e}")
                        rgb_image = None
                        depthmap = None
                        
                if rgb_image is None or depthmap is None:
                    with open('/scratch3/ma040/dataset/LMDB/incomplete.txt', 'a') as f:
                        f.write(f"arkitscenes {scene_id} {impath} {depthpath}\n")
                    new_idx = rng.integers(0, self.__len__()-1)
                    return self._get_views(new_idx, resolution, rng)
            ...
            if rgb_image.shape[:2] != depthmap.shape[:2]: 
                print(f"[ARKitScenes] RGB and depthmap shapes do not match: {rgb_image.shape} vs {depthmap.shape}")
                new_idx = rng.integers(0, self.__len__()-1)
                return self._get_views(new_idx, resolution, rng)
