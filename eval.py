...
def get_args_parser():
    ...
    parser.add_argument('--exp_path', type=str, help='Path to experiment folder', default='output/data_17k_blendmvs')
    parser.add_argument('--exp_name', type=str, default='ckpt_best', help='Path to experiment folder')
    parser.add_argument('--ckpt', type=str, default='checkpoint-best.pth', help='ckpt name')
...
def main(args):
    ...
    datasets_all = {
        '7scenes': SevenScenes(split='test', ROOT="/scratch3/ma040/dataset/7Scenes/",
                                resolution=224, num_seq=1, full_video=True, kf_every=20), #20
        'NRGBD': NRGBD(split='test', ROOT="/scratch3/ma040/dataset/NRGBD/", 
                           resolution=224, num_seq=1, full_video=True, kf_every=40),
        'DTU': DTU(split='test', ROOT="/scratch3/ma040/dataset/DTU/dtu_test_mvsnet_release/",
                   resolution=224, num_seq=1, full_video=True, kf_every=5),
    }
    model = Spann3R(dus3r_name='/scratch3/ma040/project/bkproject/spann3r_best_version/spann3r/checkpoints/DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth', 
                use_feat=False).to(args.device)
    
    # model.load_state_dict(torch.load(ckpt_path, map_location=args.device)['model'])
    model.load_state_dict(torch.load(ckpt_path, map_location=args.device, weights_only=False)['model'])
    model.eval()    
